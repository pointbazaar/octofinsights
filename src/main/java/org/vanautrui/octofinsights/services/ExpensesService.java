...
public final class ExpensesService {
    ...
    public static Record getById(final int user_id, final int expense_id) throws Exception{
        try(Connection conn= DBUtils.makeDBConnection()) {
            DSLContext create = DSL.using(conn, SQLDialect.MYSQL);
            return create.select(EXPENSES.asterisk()).from(EXPENSES).where(EXPENSES.USER_ID.eq(user_id).and(EXPENSES.ID.eq(expense_id))).fetchOne();
        }
    }
